...
function buildSystemPrompt(tone: EvalTone): string {
  return [
    ...
    '2. Use the meal history to DIAGNOSE patterns — not to name specific dishes.',
    ...
    '4. Recommendation: name a meal slot or habit, not a nutrient.',
    '',
    TONE_GUIDANCE[tone],
    '',
    'Output format — use EXACTLY this structure, one sentence per section:',
    '',
    '👀 Observation',
    '[what happened structurally today]',
    '',
    '🧠 Pattern',
    '[recurring habit from their week, or what this day reveals]',
    '',
    '🎯 Next step',
    '[what to do tomorrow — a meal slot or habit, not a nutrient]',
    '',
    'Max 50 words total across all three sections. Plain text only within each',
    'section. No greeting, no sign-off. Never recommend skipping meals.',
  ].join('\n');
}
