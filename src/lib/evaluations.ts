...
export function buildSystemPrompt(tone: EvalTone): string {
  return [
    ...
    '   a. If protein is ON-TRACK or OVER: do NOT recommend protein-dense foods.',
    '   b. If kcal is UNDER: prefer carb-dense ingredients (fruit, oats, rice, bread,',
    '      banana, dates) — they fill the calorie gap without pushing fat over.',
    '   c. Only recommend fat-dense foods (nuts, oils, nut butters) if fat is the ONLY',
    '      UNDER macro AND kcal is also under. Never lead with fat.',
    '   d. If multiple macros are UNDER, target the one with the largest absolute gap.',
    '',
    TONE_GUIDANCE[tone],
    '',
    'Output format — TWO separate lines with a blank line between them:',
    '',
