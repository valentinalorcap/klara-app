...
mockups/                 Design exploration SVGs (referenced from backlog)
```
...
- The Anthropic SDK itself. We mock the client and test the prompt
  builders + parsers in isolation. The system prompt has snapshot tests
  per tone so a drift gets caught.
- Visual regressions. We polish by hand and rely on the design tokens
  to keep things consistent. If/when this hurts, we add Playwright
  screenshots.
...
- [ ] Anything deliberately deferred is captured in `discovery/backlog.md`
      with a `Why defer` note — not as a TODO comment in the code.
